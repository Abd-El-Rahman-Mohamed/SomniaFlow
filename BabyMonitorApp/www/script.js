...
class BabyMonitorApp {
    ...
    onCordovaReady() {
        console.log('Cordova integration ready');
        // Enhanced functionality when running in Cordova
    ...
    updateMobileUI() {
        // Remove phone frame styling when running as native app
        const phoneFrame = document.querySelector('.phone-frame');
        if (phoneFrame && window.cordovaApp && window.cordovaApp.isCordova()) {
            phoneFrame.style.width = '100%';
            phoneFrame.style.height = '100vh';
    ...
    initializeDeviceFeatures() {
        ...
        if (window.cordovaApp && window.cordovaApp.isReady()) {
            console.log('Device features initialized');
            
            // Get device info
            const deviceInfo = window.cordovaApp.getDeviceInfo();
            if (deviceInfo) {
    ...
    onNetworkChange(isOnline) {
        if (isOnline) {
            this.showNotification('Network connection restored');
            // Retry video feed if it was offline
            this.loadVideoFeed();
        } else {
